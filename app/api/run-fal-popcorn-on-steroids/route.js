...
export async function POST(req) {
  try {
    ...
    const { userId, imageUrl, story } = body;
    ...
    const stream = await fal.stream(
      "workflows/MSAI-Studio-is8ypgvdt74v/popcorn-on-steriods",
      {
        input: {
          your_portrait_image:imageUrl,
          your_story: story
        },
      }
    );
    ...
    await supabase
      .from("users")
      .update({
        available_credit: user.available_credit - 100,
      })
      .eq("id", userId);
